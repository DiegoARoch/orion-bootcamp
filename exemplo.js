...
//QUESTÃO B - FUNCIONAL
function pegarNomePeloIDFuncional(id) {
    const pessoa = listaDePessoas.find((item) => item.id === id);
    return pessoa ? pessoa.name : null;
}
//QUESTÃO B - IMPERATIVO
function pegarNomePeloIDImperativo(id) {
    for (let i = 0; i < listaDePessoas.length; i++) {
        if (listaDePessoas[i].id === id) {
...
//QUESTÃO C - FUNCIONAL
function apagarItemPeloIDFuncional(id) {
...
function apagarItemPeloIDImperativo(id) {
    for (let i = 0; i < listaDePessoas.length; i++) {
        if (listaDePessoas[i].id === id) {
...
//QUESTÃO D - FUNCIONAL
function alterarItemPeloIDFuncional(id, novoNome, novaBio) {
    const pessoaIndex = listaDePessoas.findIndex((item) => item.id === id);
    if (pessoaIndex !== -1) {
        listaDePessoas[pessoaIndex] = Object.assign(Object.assign({}, listaDePessoas[pessoaIndex]), { name: novoNome !== null && novoNome !== void 0 ? novoNome : listaDePessoas[pessoaIndex].name, bio: novaBio !== null && novaBio !== void 0 ? novaBio : listaDePessoas[pessoaIndex].bio });
    }
}
//QUESTÃO D - IMPERATIVO
function alterarItemPeloIDImperativo(id, novoNome, novaBio) {
    for (let i = 0; i < listaDePessoas.length; i++) {
        if (listaDePessoas[i].id === id) {
            if (novoNome !== undefined) {
